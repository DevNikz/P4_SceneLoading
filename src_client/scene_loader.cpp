...
void SceneLoader::WorkerThread() {
    ...
    while (running_) {
        ...
        // populate model descriptors (construct ModelProgress in-place to avoid copying std::atomic)
        {
            ...
            for (int i = 0; i < manifest.models_size(); ++i) {
                ...
                // construct in-place and then initialize fields to avoid copy/move of std::atomic
                ...
                mp.parsed = false;
                // assign identity transform for now
        ...
        for (size_t i = 0; i < scene->models.size(); ++i) {
            ...
            // Progress callback updates bytes_received
            auto progress_cb = [&](int64_t got, int64_t total) {
                mp.bytes_received.store(got);
            };

            bool ok = client_->StreamModelToFile(scene->scene_id, mp.rel_path, out_path.string(), mp.size_bytes, progress_cb);
            if (!ok) {
                scene->state.store(SceneState::ERROR_STATE);
                break;
            }

            // Parse with ModelLoader into MeshData (positions/indices)
            MeshData mesh;
            // optional artificial delay to make loads observable (50 ms)
            ...
            // Compute bounding box + scale to fit into unit size and log
            ...
            // compute transformed center & radius in scene-local space (after normalization)
...

            {
                ...
                if (i < scene->model_bounds.size())
                    ...
                    // ensure vector sized (should be sized earlier)
                    if (i >= scene->model_bounds.size()) scene->model_bounds.resize(i + 1);
                    scene->model_bounds[i] = { glm::vec3(tc), transformed_radius };
                }
            }

            // Create GL upload task to run on main thread
            {
                // Move buffers into lambda to avoid copies
                ...
                // capture model_matrix by value so main thread can write it into SceneDescriptor
                ...
                upload_queue_.push([vertices = std::move(vertices), indices = std::move(indices), scene_wp, model_index = i, model_matrix, this]() mutable {
                    ...
                    if (!scene_sp) return;
                    // perform GL upload using renderer_ (must run on main GL thread)
                    MeshHandle h = renderer_->UploadMesh(vertices, indices);
                    {
                        ...
                        if (model_index < scene_sp->mesh_handles.size()) {
                            ...
                            scene_sp->model_transforms[model_index] = model_matrix; // store computed transform
                        }
                    }
            ...
            // update bytes_received to full size
            mp.bytes_received.store(mp.size_bytes);
            // mark parsed (CPU side done)
            mp.parsed = true;
        }
